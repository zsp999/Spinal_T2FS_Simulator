...
def create_model(seed):
    ...
    config = {
            ...
            'use_conv' : False, 
            'num_res_blocks':1, 
            'channel_mult':(1, 2, 4, 4, 4, 4, 8, 8),
            'num_head_channels':64,
            'use_tanh':True 
    }
    model_img = nn.SyncBatchNorm.convert_sync_batchnorm(UNetModel(**config))
    descri = nn.SyncBatchNorm.convert_sync_batchnorm(SimpleDiscriminator(in_channels=config['in_channels']+config['out_channels']))
    return model_img, descri
global_seed=525
model_img, discri_model = create_model(global_seed)
model_img.to(device)
discri_model.to(device)
...
# G_scheduler = LambdaLR(G_optimizer, lr_lambda=MyLambdaLR(epochs, 200).step)
# D_scheduler = LambdaLR(D_optimizer, lr_lambda=MyLambdaLR(epochs, 200).step)
...
for epoch in range(epochs):  
        ...
        for data_iter_step, data_batch in enumerate(train_loader):
                ...
                realpatch_label, fakepatch_label = torch.ones((label_data.shape[0],1,32,32), dtype=torch.float32, device=device), \
                        torch.zeros((label_data.shape[0],1,32,32), dtype=torch.float32, device=device)
                ...
                with autocast():
                        ...
                        loss_gan = criterion_GAN(discri_model(fake_img,img_data.to(device)), realpatch_label)
                ...
                with autocast():
                        loss_real = criterion_GAN(discri_model(label_data.to(device),img_data.to(device)), realpatch_label)
                        loss_fake = criterion_GAN(discri_model(fake_img.detach(),img_data.to(device)), fakepatch_label) 
        ...
        if (epoch+1)%10==0:
                ...
                draw_on_test(model_img, val_loader, f"{save_name}/pict_{changes}", epoch, device, name='pix2pix')
